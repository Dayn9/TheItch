...
public class DebufferCollider : MonoBehaviour {
    ...
    public void Debuffer()
    {
        ...
        if (box != null)
        {
            box.offset += new Vector2(Global.BUFFER, Global.BUFFER);
            box.size -= new Vector2(Global.BUFFER, Global.BUFFER);
        }
        ...
        if (circle != null)
        {
            circle.offset += new Vector2(Global.BUFFER, Global.BUFFER);
            circle.radius -= Global.BUFFER;
        }
        ...
        if (poly != null)
        {
            ...
            for (int i = 0; i < poly.points.Length; i++)
            {
                ...
                newPoints[i] = poly.points[i] - poly.points[i].normalized * (2 * Global.BUFFER);
